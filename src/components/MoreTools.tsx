...
import { Chrome, Layers, ExternalLink, BookmarkCheck, ArrowRight, ShieldCheck, Check, Clipboard } from "lucide-react";
    ...
    <div className="space-y-10 py-4 animate-fade-in" id="more-tools-root">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8" id="more-tools-grid">
        ...
        <div className="lg:col-span-7 space-y-6" id="downloader-extensions">
          <h3 className="font-semibold text-xs text-gray-400 uppercase tracking-widest pl-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6" id="extensions-subgrid">
            {extraTools.map((tool, idx) => (
              <div
                key={idx}
                className="flex flex-col justify-between overflow-hidden rounded-3xl border border-gray-100 bg-white p-6 shadow-xs hover:shadow-md transition-all duration-300"
              >
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    {tool.status === "active" ? (
                      <span className="inline-flex items-center gap-1.5 text-xs font-semibold text-emerald-500 font-mono">
                        <span className="h-1.5 w-1.5 rounded-full bg-emerald-500 animate-pulse" />
                        ACTIVE
                      </span>
                    ) : (
                      <span className="inline-flex items-center gap-1.5 text-xs font-semibold text-gray-400 font-mono">
                  ...
                  <div className="space-y-1.5">
                    <h4 className="font-semibold font-display text-gray-950 text-base">
                      {tool.name}
                    </h4>
                ...
                <div className="pt-6 border-t border-gray-50 mt-4 flex items-center justify-between">
                  <span className={`text-xs font-semibold flex items-center gap-1.5 ${tool.status === "active" ? "text-blue-600" : "text-gray-400"}`}>
                  ...
                  <button className={`flex h-8 w-8 items-center justify-center rounded-lg transition-colors ${
                    tool.status === "active" 
                      ? "bg-gray-50 hover:bg-blue-50 text-gray-400 hover:text-blue-600 cursor-pointer" 
        ...
        <div className="lg:col-span-5 space-y-6" id="bookmarklet-section">
          ...
          <div className="rounded-3xl border border-gray-100 bg-white p-6 md:p-8 shadow-xs space-y-6">
            <div className="space-y-3">
              ...
              <p className="text-xs text-gray-500 leading-relaxed font-light">
                Download social media files without even copying links! Drag our smart bookmarklet link below onto your web bookmarks bar, and click it whenever you are viewing any Tweet or Instagram video.
              </p>
            </div>

            {/* Draggable drag CTA card */}
            <div className="rounded-2xl border border-blue-50 bg-blue-50/20 p-4 text-center space-y-3">
            ...
            <div className="space-y-2 pt-2 border-t border-gray-50">
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="flex-grow text-[10px] font-mono text-gray-400 bg-gray-50 border border-gray-100 px-3 py-2 rounded-xl focus:outline-none"
                />
                <button
                  onClick={copyBookmarklet}
                  className="shrink-0 flex items-center justify-center h-8.5 w-8.5 rounded-xl border border-gray-100 bg-white hover:bg-gray-50 text-gray-500 cursor-pointer"
